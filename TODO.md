...
- [ ] **Extract `DeckListItem` Component:** Refactor `app/pages/library.vue` and `app/pages/shared.vue` to use a shared `DeckListItem.vue` component for displaying deck cards.
- [ ] **Extract `DeckFilterToolbar` Component:** Refactor `app/pages/library.vue` and `app/pages/shared.vue` to use a shared `DeckFilterToolbar.vue` for the search/filter UI.
...
- [ ] **Empty States:** `app/pages/library.vue` has a basic empty state. Improve visual feedback when a user has no decks.
